...
| **P15** | IPC wire format migration | *Not designed yet — deliberately last, after every other phase including P14.* Placeholder so the request is not lost: replace today's JSON-shaped `structured clone` wire encoding (§3.5) with FlatBuffers, for lower serialization overhead on large payloads (e.g. graph streaming). Scope, versioning and migration path are to be decided at design time, not here. | *To be defined at design time.* |
