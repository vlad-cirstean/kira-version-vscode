...
| D20 | Signature verification | **Selected commit only.** `--show-signature` per commit is far too slow in bulk and a per-row trust badge is not worth a process spawn per visible row. **Gap, not a decision:** §7.9's tag table also names `-s` (signed tag creation, gated on `tag.gpgSign`) and tag signature verification "under the same rule" — P6 shipped `TagDialog` without either, and no phase's exit criteria tests them. Small, cheap, unphased; pick up whenever a phase next touches tag creation. |
